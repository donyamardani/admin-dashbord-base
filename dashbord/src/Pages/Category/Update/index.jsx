...
export default function UpdateCategory() {
  ...
  useEffect(() => {
    (async () => {
      ...
      if (result?.data?.[0]) {
        ...
        setSupCategoryId(category.supCategoryId?._id || category.supCategoryId || "");
        if (category.image) {
          setImg([
            {
              id: 1,
  ...
  // گرفتن لیست دسته‌بندی‌ها برای انتخاب دسته‌بندی والد (به جز خود این دسته)
  useEffect(() => {
    (async () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (result.success) {
        notify("success", result.message || "دسته‌بندی با موفقیت ویرایش شد");
        navigate("/dashboard/category");
      } else {
        if (result.status !== 401 && result.status !== 403) {
          notify("error", result.message || "خطا در ویرایش دسته‌بندی");
        }
      }
    } catch (err) {
  ...
  return (
    <div className="max-w-2xl mx-auto p-4 sm:p-0" dir="rtl">
      <div className="mb-6 flex items-start justify-between gap-3 border-b border-gray-100 pb-4">
        <div>
          <h2 className="text-xl sm:text-2xl font-bold text-gray-800">ویرایش دسته‌بندی</h2>
          <p className="mt-1 text-sm text-gray-500">مشخصات، تصویر و وضعیت انتشار دسته‌بندی را تغییر دهید.</p>
        </div>
        <button
          type="button"
          onClick={() => navigate(-1)}
      ...
      <form onSubmit={handleSubmit} className="rounded-2xl border border-gray-100 bg-white shadow-sm p-5 sm:p-6 space-y-6">
        <div className="space-y-2">
          <label className="text-sm font-semibold text-gray-700 block">عنوان دسته‌بندی <span className="text-red-500">*</span></label>
          <input
            type="text"
            name="title"
            value={title}
        ...
        <div className="space-y-2">
          <label className="text-sm font-semibold text-gray-700 block">دسته‌بندی والد (اختیاری)</label>
          <select
            value={supCategoryId}
          ...
          <p className="text-xs text-gray-400">
            توجه: یک دسته‌بندی نمی‌تواند والد خودش باشد، به همین دلیل از این لیست حذف شده است.
          </p>
        </div>

        <div className="space-y-2">
          <div className="flex items-center justify-between gap-2">
            <label className="text-sm font-semibold text-gray-700">تصویر دسته‌بندی (اختیاری)</label>
        ...
        <div className="flex items-center justify-between gap-3 rounded-xl border border-gray-100 bg-gray-50 p-4">
          <div>
            ...
            <p className="text-xs text-gray-500 mt-0.5">در صورت فعال بودن، این دسته‌بندی در سایت عمومی نمایش داده خواهد شد.</p>
          </div>
