...
export default function GetAllProduct() {
  ...
  const totalPages = Math.ceil(countOfProducts / limit) || 1;

  useEffect(() => {
    (async () => {
      const result = await FetchData(`products?page=${page}&limit=${limit}&sort=${sort}${search ? `&search=${search}` : ''}`, {
        method: 'GET',
      })
      if (result) {
        setProducts(result.data)
        setCountOfProducts(result.count)
      }
    })()
  ...
  const handleRemove = async (id) => {
    ...
    if (result.success) {
      const newProducts = products?.filter((item) => item._id.toString() !== id.toString())
  ...
  const items = products?.map((item, index) => (
    <tr key={item._id} className="border-b border-gray-100 hover:bg-gray-50/70 transition duration-150">
      ...
      <td className="px-6 py-4 text-sm text-gray-800 font-semibold max-w-[220px] truncate">{item?.title}</td>
      <td className="px-6 py-4 text-sm text-gray-600">{item?.brandId?.title || '-'}</td>
      <td className="px-6 py-4 text-sm text-gray-600">{item?.categoryId?.title || '-'}</td>
      <td className="px-6 py-4 text-sm text-gray-600 whitespace-nowrap">
        {item?.minPrice || item?.maxPrice
          ? `${formatPrice(item.minPrice)} ${item.maxPrice && item.maxPrice !== item.minPrice ? `تا ${formatPrice(item.maxPrice)}` : ''} تومان`
          : '-'}
      </td>
      <td className="px-6 py-4 text-sm">
        {item?.isPublished ? (
      ...
      <td className="px-6 py-4 text-sm">
        <div className="flex items-center gap-2">
          <button
            onClick={() => navigate(`/dashboard/product/update/${item?._id}`)}
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 bg-white p-4 rounded-2xl border border-gray-100 shadow-sm">
        ...
        <div className="relative flex items-center gap-2 min-w-[200px]">
          ...
          >
            ...
            <option value="minPrice">ارزان‌ترین‌ها</option>
            <option value="-minPrice">گران‌ترین‌ها</option>
          </select>
        </div>
      </div>
      ...
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
          ...
          <table className="w-full text-right border-collapse">
            <thead>
              <tr className="bg-gray-50/75 border-b border-gray-100">
                ...
                <th className="px-6 py-3.5 text-sm font-bold text-gray-500">عنوان محصول</th>
                <th className="px-6 py-3.5 text-sm font-bold text-gray-500">برند</th>
                <th className="px-6 py-3.5 text-sm font-bold text-gray-500">دسته‌بندی</th>
                <th className="px-6 py-3.5 text-sm font-bold text-gray-500">محدوده قیمت</th>
            ...
            <tbody>
              {products.length > 0 ? (
                ...
                <tr>
                  <td colSpan="8" className="px-6 py-10 text-center text-sm text-gray-400 font-medium">
                    هیچ محصولی یافت نشد.
                  </td>
                </tr>
              )}
        ...
        <div className="px-6 py-4 bg-gray-50/70 border-t border-gray-100 flex flex-col sm:flex-row items-center justify-between gap-4 text-sm text-gray-600">
          <div className="font-medium text-xs sm:text-sm">
            نمایش داده‌های تکه {products.length > 0 ? (page - 1) * limit + 1 : 0} تا {Math.min(page * limit, countOfProducts)} از مجموع <span className="font-bold text-teal-600">{countOfProducts}</span> محصول
          </div>
